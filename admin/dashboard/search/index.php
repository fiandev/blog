...
$totalData = $db->countRows("SELECT * FROM article WHERE 
title LIKE '%$q%' OR
content LIKE '%$q%'
");
$dataPerHal = 15;
...
$rows = $db->assoc("SELECT * FROM article WHERE
 title LIKE '%$q%' OR
 content LIKE '%$q%'
 ORDER BY  id DESC LIMIT $awalData, $dataPerHal");
$dump = [];
?>
<!doctype html>
<?php if(isset($_GET["q"])) :?>
  <?php if($totalData < 1): ?>
  <h5>tidak ditemukan hasil untuk '<?= $q; ?>'</h5>
  <?php else: ?>
  ...
  <div class="row">
    ...
    <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
      ...
      <div class="table-responsive">
        ...
        <h5>tidak ditemukan hasil untuk '<?= $q; ?>'</h5>
        <?php else: ?>
        <h5>hasil di temukan untuk '<?= $q; ?>'</h5>
        <table class="table table-striped table-sm">
          <thead>
            <tr class="text-center">
              <th scope="col">no</th>
