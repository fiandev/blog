...
if (isset($_GET["id"]) && isset($_GET["action"])) {
  ...
  switch ($_GET["action"]) {
    case 'delete':
      ...
      break;
    case 'update':
      $article = $db->field("SELECT * FROM article WHERE id=$id");
      if ($article == null) {
        $page->_404();
      }
      break;
...
<div class="container-fluid">
  <div class="row">
    ...
    <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
      <h5 class="my-2">Update Article</h5>
      <form action="" method="post" enctype="multipart/form-data" class="form-control">
          <label for="writer">penulis:</label>
          <input required  class="form-control" type="text" name="writer" id="writer" value="<?= $article["writer"]; ?>" />
          ...
          <input class="form-control" type="file" accept="image/*" name="thumbnail" id="thumbnail" value="" />
          <label for="content">content:</label>
          <textarea required style="height:40vh" class="form-control" name="content" id="content" value="<?= $article["content"]; ?>"><?= $article["content"]; ?></textarea>
